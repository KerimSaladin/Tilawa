...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($teacher_id) {
        if ($action === 'approve') {
            ...
            $pdo->prepare("UPDATE users SET has_star = TRUE - has_star WHERE id=?")->execute([$teacher_id]);
            $_SESSION['flash_success'] = 'تم تحديث نجمة المعلم.';
        } elseif ($action === 'toggle_exam') {
            $pdo->prepare("UPDATE users SET exam_passed = TRUE - exam_passed WHERE id=? AND user_type='teacher'")->execute([$teacher_id]);
