...
function register_user($pdo, $data) {
    ...
    $cols = ['email','password_hash','full_name','user_type','teacher_status','exam_required','gender','phone','quran_certificate','trial_end_date','payment_method_token'];
    ...
    try {
        ...
        if ($user_type === 'teacher' && $has_certificate) {
            ...
            $upload_result = upload_file($data['quran_certificate'], 'certificate', $user_id);
            ...
            $certificate_path = USE_R2 ? $upload_result['url'] : $upload_result['path'];

            $update_stmt = $pdo->prepare("UPDATE users SET quran_certificate = ? WHERE id = ?");
            $update_stmt->execute([$certificate_path, $user_id]);

            $file_extension = strtolower(pathinfo($upload_result['path'], PATHINFO_EXTENSION));
            $file_type = in_array($file_extension, ['pdf']) ? 'application/pdf' : 'image/' . $file_extension;
            $pdo->prepare("INSERT INTO teacher_files (teacher_id,file_name,file_path,file_type,description) VALUES (?,?,?,?,?)")
                ->execute([$user_id, basename($certificate_path), $certificate_path, $file_type, 'شهادة القرآن الكريم']);
        }
