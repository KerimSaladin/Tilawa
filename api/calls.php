...
try {
    if ($action === 'create' && $method === 'POST') {
        ...
        $stmt = $pdo->prepare("INSERT INTO calls (caller_id, callee_id, type, status) VALUES (?, ?, ?, 'ringing') RETURNING id");
    ...
    if ($action === 'add_candidate' && $method === 'POST') {
        ...
        $stmt = $pdo->prepare("INSERT INTO call_candidates (call_id, sender_id, candidate) VALUES (?, ?, ?) RETURNING id");
