...
try {
    ...
    if ($action === 'check_incoming' && $method === 'GET') {
        $stmt = $pdo->prepare("SELECT c.*, u.full_name AS caller_name FROM calls c JOIN users u ON c.caller_id = u.id WHERE c.callee_id = ? AND c.status = 'ringing' AND c.created_at > DATE_SUB(NOW(), INTERVAL 30 SECOND) ORDER BY c.created_at DESC LIMIT 1");
