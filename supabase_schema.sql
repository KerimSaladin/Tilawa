...
-- ── Add missing columns to progress (run if table already exists) ─────────────
ALTER TABLE progress ADD COLUMN IF NOT EXISTS teacher_id INT REFERENCES users(id) ON DELETE SET NULL;
ALTER TABLE progress ADD COLUMN IF NOT EXISTS completion_pct DECIMAL(5,2) DEFAULT 0;
