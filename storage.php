...
function _r2_client() {
    ...
    // Supabase Storage: virtual-hosted style, explicit region
    // Cloudflare R2: path-style, region=auto
    $isSupabase = str_contains(R2_ENDPOINT, 'supabase.co');
    $client = new Aws\S3\S3Client([
        'version'                 => 'latest',
        'region'                  => $isSupabase ? 'ap-southeast-1' : 'auto',
        'endpoint'                => R2_ENDPOINT,
        'credentials'             => ['key' => R2_KEY, 'secret' => R2_SECRET],
        'use_path_style_endpoint' => !$isSupabase,
    ]);
    return $client;
}
