...
$chat_user = null;
if ($chat_uid > 0) {
    $stmt = $pdo->prepare("SELECT id, full_name, user_type, is_online, last_seen FROM users WHERE id=? AND is_active=1");
    $stmt->execute([$chat_uid]);
    $chat_user = $stmt->fetch();
    if ($chat_user) {
        $pdo->prepare("UPDATE messages SET is_read=1 WHERE sender_id=? AND receiver_id=?")->execute([$chat_uid, $user['id']]);
    }
}
...
$conversations = $pdo->prepare("
    SELECT DISTINCT
        ...
        (SELECT COUNT(*) FROM messages WHERE sender_id=other_id AND receiver_id=? AND is_read=0) as unread
    FROM messages
